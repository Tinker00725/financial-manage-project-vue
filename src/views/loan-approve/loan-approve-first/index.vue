...
export default {
  ...
  methods: {
    ...
    // 搜索数据2 获取数据
    async getData() {
      await this.$store.dispatch("first/getFirstUser", this.info);
    },
    // 搜索数据1 更新name
    async updateName(str) {
      await this.$store.commit("first/UPDATAINFONAME", str);
    },
    // 显示编辑框
    showEditBox(row){
    ...
    async approveFirstPass(row){
      this.$confirm('此操作将通过当前初审对象, 是否继续?', '提示', {
          confirmButtonText: '确定',
          cancelButtonText: '取消',
          type: 'warning'
        }).then(async () => {
          try{
        await this.$store.dispatch('first/passUserLoanFirst',row.id)
        this.getData();
    }catch(e){
        console.log(e)
      }
    ...
    async approveFirstReject(row){
      this.$confirm('此操作将拒绝当前初审对象, 是否继续?', '提示', {
          confirmButtonText: '确定',
          cancelButtonText: '取消',
          type: 'warning'
        }).then(async() => {
          await this.$store.dispatch('first/rejectLoanUser',row.id)
          this.getData();
        }).catch(() => {
          this.$message({
            type: 'info',
